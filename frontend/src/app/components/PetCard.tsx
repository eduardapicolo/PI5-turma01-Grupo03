...
function formatarDistancia(distancia?: number): string | null {
...
export function PetCard({ pet, onClick }: PetCardProps) {
  ...
  const distancia = formatarDistancia(pet.distancia);
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-square overflow-hidden bg-muted flex-shrink-0">
        ...
        {distancia && (
          <div className="absolute top-3 right-3 rounded-xl bg-black/70 px-3 py-2 text-right text-white shadow-md backdrop-blur-sm">
            <p className="text-[10px] font-medium leading-none opacity-80">
              Distância euclidiana
            </p>
            <p className="mt-1 text-sm font-bold leading-none">{distancia}</p>
          </div>
        )}
      </div>
